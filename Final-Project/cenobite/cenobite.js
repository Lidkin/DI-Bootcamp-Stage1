...
let baseRadius = 5;
//let maxTornLength = 200;

const sketch = ({ context, width, height }) => {
    let centerX = width * 0.5 - 15; // -15 for bwFace1.png
    ...
    //---------------------torns map-----------------------//
    //context.translate(centerX, centerY); needed for first mapping torns

    for (let i = 0; i < 4; i++) {
        if (i == 1) { xm = -1; ym = 1; }
        if (i == 2) { xm = 1; ym = -1; }
        if (i == 3) { xm = -1; ym = -1; }
        //context.setTransform(xm, 0, 0, ym, centerX, centerY); for mapping
    ...
    for (let i = 0; i < 2; i++) { //face perimeter vertical
        ...
        xm = -1; ym = 1;
        if (i == 1) { xm = 1; ym = -1; }
        //context.setTransform(xm, 0, 0, ym, centerX, centerY);
        for (let i = 0; i < (numCols + numRows); i++) {
...
const tornColor = (x, y, centerX) => {
    ...
    let r = x < (centerX - 100) ? mapRange(radiusR, 0, 500, 255, 0) : mapRange(radiusR, 20, 400, 255, 50);
...
const createAudio = () => {
    ...
    //'https://cdn.pixabay.com/download/audio/2022/12/29/audio_81cb15f0d0.mp3?filename=scary-horror-trailer-exploring-other-worlds-131560.mp3';
...
const start = async () => {
    imgA = await loadImage('cenobite/bwFace1.png'); //face6
    //imgA = await loadImage('https://cdn.pixabay.com/photo/2019/08/13/06/39/woman-4402587_1280.jpg');
